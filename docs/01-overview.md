...
| F7 | Notes-app style editing (WYSIWYG toolbar over the Markdown subset per [doc 06](06-storage-format.md)); Markdown rendering in reader; a date-context header and direct first-entry action in the empty timeline; two-line plain-text previews and visible entry times in populated timelines; packaged Supreme body text and selectively used Chubbo headings; restrained solid-color surfaces, semantic primary actions, clearly bounded action controls, borderless large Back icons, visible keyboard focus, and tactile press states give the UI depth without gradients, with accent surfaces adapted for both light and dark themes and centered width limits on larger displays |
